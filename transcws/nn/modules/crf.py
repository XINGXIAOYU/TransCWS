...
class CRF(nn.Module):
    def __init__(self, **kwargs):
        ...
        init_trainsitions = torch.zeros(self.target_size + 2, self.target_size + 2)
        init_trainsitions[:, self.START_TAG_IDX] = -1000
        init_trainsitions[self.END_TAG_IDX, :] = -1000
        if self.use_cuda:
            init_trainsitions = init_trainsitions.cuda()
        self.transitions = nn.Parameter(init_trainsitions)
